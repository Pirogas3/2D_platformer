...
namespace Assets.Scripts.UI.Widgets
{
    ...
    private void Awake()
    {
        ...
        {
            // Предмет инвентаря
            var itemData = itemCell.GetItemData(); // нужно добавить публичный метод GetItemData в InventoryItemCell
            if (itemData != null)
            {
                _id = itemData.Id;
                ...
                {
                    // Для предметов можно взять иконку монеты, если есть стоимость
                    // _costIcon = ... (можно использовать иконку валюты из Defs)
                }
            }
            return;
        }
        ...
        if (perkSlot != null)
        {
            ...
            {
                // Можно взять иконку стоимости из PerkDef (если добавить поле Icon)
                // _costIcon = perkDef.Icon;
            }
            return;
        }
