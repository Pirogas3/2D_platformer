namespace SheetXExample
{
	...
	public class LocalizationUIText : MonoBehaviour
	{
	    ...
	    [SerializeField, HideInInspector] private LocalizationUI.ID m_LocalizedEnumId = LocalizationUI.ID.NONE; //Used to search for the usage of an ID within the project
	    ...
	    public LocalizationUI.ID LocalizedEnumId => m_LocalizedEnumId;
	    ...
	    {
	        if (LocalizationUI.CurLangIndex > -1)
	            Refresh();
			
			LocalizationUI.OnLanguageChanged += OnLanguageChanged;
	    }
	    ...
	    public void Set(int pId, params string[] pArgs)
	    {
	        ...
	        {
	            m_LocalizedID = pId;
	            m_LocalizedEnumId = (LocalizationUI.ID)pId;
	            m_LocalizedIDString = LocalizationUI.idString[m_LocalizedID];
	        }
			m_Args = pArgs;
			Refresh();
	    }
	    ...
	    public void Set(string pId, string[] pArgs)
	    {
	        if (m_LocalizedIDString != pId)
	        {
	            ...
	            m_LocalizedID = -1;
	            m_LocalizedEnumId = LocalizationUI.ID.NONE;
	        }
			m_Args = pArgs;
			Refresh();
	    }
	    ...
	    public void Refresh()
	    {
			int CurLangIndex = LocalizationUI.CurLangIndex;
	        if (Application.isPlaying && m_LanguageIndex == CurLangIndex)
				return;
			if (m_IgnoredLangs.Count > 0 && m_IgnoredLangs.Contains(LocalizationUI.CurrentLanguage))
				return;
	        string text = "";
	        if (m_LocalizedID >= 0)
	            text = LocalizationUI.Get(m_LocalizedID).ToString();
	        else if (!string.IsNullOrEmpty(m_LocalizedIDString))
	            text = LocalizationUI.Get(m_LocalizedIDString, ref m_LocalizedID).ToString();
	        switch (m_CaseType)
	        {
	    ...
	    private void OnValidate()
	    {
			...
			int localizedID = -1;
	        LocalizationUI.InitInEditor();
			LocalizationUI.Get(m_LocalizedIDString, ref localizedID);
			if (localizedID == -1)
			{
				string path = name;
				...
				Debug.LogError($"{path}: not found {m_LocalizedIDString} in {nameof(LocalizationUI)}");
			}
	    }
	    ...
	    {
	        private LocalizationUIText m_Target;
	        ...
	        private void OnEnable()
	        {
	            LocalizationUI.InitInEditor();
	            m_MaxResults = 20;
	            m_Target = target as LocalizationUIText;
	        }
	        ...
	        public override void OnInspectorGUI()
	        {
				...
				if (LocalizationUI.CurrentLanguage != LocalizationUI.DefaultLanguage && GUILayout.Button("Reset Default Language"))
	            {
					LocalizationUI.CurrentLanguage = LocalizationUI.DefaultLanguage;
					m_Target.Refresh();
				}
				...
				if (m_Target.m_IgnoredLangs.Count > 0)
				{
					for (int i = 0; i < m_Target.m_IgnoredLangs.Count; i++)
					{
						...
						{
							m_IgnoredLangIdx--;
							m_IgnoredLangIdx = Mathf.Clamp(m_IgnoredLangIdx, 0, LocalizationUI.LanguageFiles.Count - 1);
							m_Target.m_IgnoredLangs[i] = LocalizationUI.GetLanguage(m_IgnoredLangIdx);
						}
						if (GUILayout.Button(">"))
						{
							m_IgnoredLangIdx++;
	            ...
	            {
	                int index = 0;
	                foreach (var lang in LocalizationUI.LanguageFiles)
	                {
	                    if (lang.Key == LocalizationUI.CurrentLanguage)
	                        break;
	                    index++;
	                }
	
	                index = (index + 1) % LocalizationUI.LanguageFiles.Count;
	                ...
	                foreach (var lang in LocalizationUI.LanguageFiles)
	                {
	                    if (index2 == index)
	                    {
	                        LocalizationUI.CurrentLanguage = lang.Key;
	                        m_Target.Refresh();
	                        break;
	                    }
	                    index2++;
	            ...
	            for (int i = 0; i < LocalizationUI.idString.Length; i++)
	            {
	                ...
	                bool contain = false;
	                string idString = LocalizationUI.idString[i];
	                ...
	                if (contain)
	                {
	                    string additional = "";
	                    if (m_ShowText)
	                        additional = LocalizationUI.Get(idString).ToString();
	                    ...
	                    if (GUILayout.Button(buttonName, style))
	                    {
	                        ...
	                        m_Target.m_LocalizedEnumId = LocalizationUI.ID.NONE;
	                        m_Target.Init();
	                        m_Target.Refresh();
	                    }
